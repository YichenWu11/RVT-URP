...
struct Varyings_VT
{
    ...
    #if!defined(TERRAIN_SPLAT_BASEPASS) && !defined(ENABLE_RVT)
...
void ComputeMasks(out half4 masks[4], half4 hasMask, Varyings_VT IN)
{
    masks[0] = 0.5h;
    masks[1] = 0.5h;
    masks[2] = 0.5h;
...
void InitializeInputData_Initial(Varyings_VT IN, half3 normalTS, out InputData inputData)
{
...
void InitializeInputData_VT(Varyings_VT IN, half3 normalTS, out InputData inputData)
{
    inputData = (InputData)0;
...
// Vertex Shader
Varyings_VT SplatmapVert_VT(Attributes_VT v)
{
    Varyings_VT o = (Varyings_VT)0;
    ...
    Attributes.positionWS = TransformObjectToWorld(v.positionOS);
    Attributes.positionCS = TransformWorldToHClip(Attributes.positionWS);
...
#else
half4 SplatmapFragment_VT(Varyings_VT IN) : SV_TARGET
#endif
{
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN);
    #ifdef _ALPHATEST_ON
    ...
    // float4 screenUVParam = half4(1, 1, 1, 1);
    // screenUVParam.xy = _ScreenResolution * virtualUV;

    /*
     *   Implementation of Runtime Virtual Texture
     *    
     *   support albedo map and normal map
     */
    #if defined(ENABLE_RVT)
    int mipLevel;
    FinalizeFeedbackBuffer(IN.clipPos, virtualUV, mipLevel);
    RVTData rvt_data;
    SamplePhysicalTexture(virtualUV, rvt_data);
    half3 albedo = rvt_data.albedo;
    normalTS = rvt_data.normal;
    half metallic = rvt_data.metallic;
    half smoothness = rvt_data.smoothness;
    half alpha = 1;
    half occlusion = 1;
    #elif defined(TERRAIN_SPLAT_BASEPASS)
    ...
    InputData inputData;
    // InitializeInputData_VT(IN, normalTS, inputData);
    InitializeInputData_Initial(IN, normalTS, inputData);
...
VaryingsLean ShadowPassVertex_VT(AttributesLean v)
{
...
half4 ShadowPassFragment_VT(VaryingsLean IN) : SV_TARGET
{
    #ifdef _ALPHATEST_ON
...
VaryingsLean DepthOnlyVertex_VT(AttributesLean v)
{
    VaryingsLean o = (VaryingsLean)0;
    UNITY_SETUP_INSTANCE_ID(v);
    UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o);
...
half4 DepthOnlyFragment_VT(VaryingsLean IN) : SV_TARGET
{
    #ifdef _ALPHATEST_ON
    ClipHoles(IN.texcoord);
    #endif
