...
min16uint4 UintToHalf(in uint2 xy)
{
    min16uint x = min16uint(xy.x & 0x0000ffff);
    min16uint y = min16uint((xy.x & 0xffff0000) >> 16);
    min16uint z = min16uint(xy.y & 0x0000ffff);
    min16uint w = min16uint((xy.y & 0xffff0000) >> 16);
    return min16uint4(x,y,z,w);
}
